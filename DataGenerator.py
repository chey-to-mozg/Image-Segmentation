...
class Dataset:
    ...
    def __getitem__(self, i):
        ...
        image = cv2.imread(self.images_fps[i])  # [:,:,0] #cv2.imread(self.images_fps[i], -1)
        ...
        mask = cv2.imread(self.masks_fps[i])  # cv2.imread(self.masks_fps[i], 0)
        if self.in_one_file:
            mask = mask[:, int(mask.shape[1] / 2):, :]
